	...
	public class Set {
		...
		public static List collect_0(List _l, CDO cdox, int k, int m) {
			...
			for (int _i = 0; _i < _l.size(); _i++) {
				...
				_results_0.add(new Double(((Sector) cdox.getsectors()
						.get(k - 1)).contagion(i, m)));
			}
			return _results_0;
		}
			...
			for (int _i = 0; _i < _l.size(); _i++) {
				...
				double dd = new Double(
				      ...
				      * ((Sector) cdox.getsectors().get(k - 1)).getL()
			     	  * cdox.PCond(k, mk) 
			     	  * cdox.PS((int)(s - mk * ((Sector) cdox.getsectors().get(k - 1)).getL())
			     	           )
			          );
				
				_results_2.add(dd);
				
//				System.out.println("PCond(k, mk): " + cdox.PCond(k, mk) + "  PS: " + cdox.PS((int)(s - mk * ((Sector) cdox.getsectors().get(k - 1)).getL()) ) );
		...
		public static int count(List l, Object obj) {
			...
			for (int _i = 0; _i < l.size(); _i++) {
				if (obj == l.get(_i)) {
					res++;
				} else if (obj != null && obj.equals(l.get(_i))) {
					res++;
				}
			}
			return res;
